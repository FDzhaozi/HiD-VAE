...
class SemanticIdTokenizer(nn.Module):
    ...
    def __init__(
        ...
        use_dedup_dim: bool = True  # 新增参数，控制是否使用去重维度
    ) -> None:
        super().__init__()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='测试语义ID标记器')
    parser.add_argument('--dataset', type=str, default='ml-1m', choices=['ml-1m', 'beauty'],
    ...
    print(f"模型参数: input_dim={args.input_dim}, embed_dim={args.embed_dim}, "
          ...
          f"n_cat_feats={args.n_cat_feats}, use_dedup_dim={args.use_dedup_dim}")
    ...
    tokenizer = SemanticIdTokenizer(
        ...
        n_layers=3,
        rqvae_weights_path="out/rqvae/amazon/checkpoint_best.pt",
