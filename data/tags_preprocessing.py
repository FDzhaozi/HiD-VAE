...
# Processes movie genre data.
# If `one_hot=True`, returns the original data directly; otherwise, converts movie genres into a list of indices.
...
# # 2. Sliding Window for User History Generation
# **_rolling_window**:
...
# **_df_to_tensor_dict**:
# Converts a `polars.DataFrame` into a dictionary of `torch.Tensors`.
# Processes feature data and future features (e.g., `movieId_fut`, `rating_fut`).

# # 5. Generating User History
# **_generate_user_history**:
# The main method that integrates the above steps to generate user history.
# Specific process:
# Groups by user and sorts by timestamp.
# Generates user history using a sliding window.
# Pads records that are smaller than the window size.
